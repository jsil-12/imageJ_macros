...
function iter_circles_and_measure(X,Y,radius,n,lower,upper,mainWindow,measurement){
	for (i=1; i<(n+1); i++){
		selectWindow(mainWindow);
		run("Duplicate...", "title=iter_citcle.tif");
		selectWindow("iter_citcle.tif");
		wait(10);
		...
		if(measurement == 'measure'){
			...
			wait(5);
			setResult("radius_iter", row1 , value);
			updateResults();
			setResult("radius_iter", row2 ,value);
			updateResults();
			print(row1);
			print(row2);
...
function per_file_execution(xy_coord_dir, output_dir, file, radius, n, measurement){
	...
	if( N > 1) {
		...
		for (i = 1; i < N; i++){
			...
			save_results(i,output_dir,f_name);
		}
		while(N > 0) {
			roiManager("Select",0);
			roiManager("Delete");
...
function main(bool){
	...
	output_dir = "F:\\lab_files\\imageJ_macro_working_directory\\Gfap_Iba1_ki67\\microglia_astrocyte_7D_output\\whole_output\\radial_analysis\\features\\microglia\\";
	radius = 200;
	n = 20;
	measurement = "measure";
